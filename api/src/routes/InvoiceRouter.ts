...
RequestManager.post(
  ...
  RequestManager.asyncResolver(
    async (
      ...
      const genreatedLog = new InvoiceLogEntity();
      genreatedLog.invoice = invoice;
      genreatedLog.client = invoice.client;
      genreatedLog.code = InvoiceState.GENERATED;
      genreatedLog.details = `Invoice ${invoice.number} has been generated by ${request.user?.firstname}`;
      await genreatedLog.save();
      ...
      pdf
        ...
        .text("Prix (CHF)", 350, tableTop + 5, { width: 50, align: "right" })
        .text("Montant (CHF)", 450, tableTop + 5, {
          width: 70,
          align: "right",
        });
      ...
      invoice.invoiceServices.forEach((s) => {
        ...
        const lineTotal = s.amount * s.quantity;
        ...
        pdf
          ...
          .text(s.amount.toFixed(2), 350, y + 5, { width: 50, align: "right" })
          .text((s.amount * s.quantity).toFixed(2), 450, y + 5, {
            width: 70,
            align: "right",
          });
      ...
      pdf
        .font("Outfit-Bold")
        .fontSize(9)
        .text("TOTAL À PAYER (CHF)", 350, yTotal, {
          width: 100,
          align: "right",
        })
        .text(`${total.toFixed(2)}`, 450, yTotal, {
          width: 70,
          align: "right",
        });
