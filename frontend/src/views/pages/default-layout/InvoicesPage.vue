  ...
  return invoices.value
    .filter((i: InvoicePage) => {
      ...
      if (
        ...
        !selectedInvoiceNumbers.value.includes(i.number)
      )
        return false
      if (selectedInvoiceNames.value.length > 0 && !selectedInvoiceNames.value.includes(i.name))
        return false
      if (selectedInvoiceStates.value.length > 0 && !selectedInvoiceStates.value.includes(i.state))
        return false
    ...
    .sort((a, b) => {
      ...
      if (selectedSort.value === 'recent')
        return (
          new Date(b.logs[0]?.createdAt || 0).getTime() -
          new Date(a.logs[0]?.createdAt || 0).getTime()
        )
  ...
  <div class="bg-lightBlack p-6 rounded-xl mb-5">
    ...
    <div class="flex flex-wrap gap-4 mt-6 mb-4">
      ...
      <AmountRangeSelect
        ...
        v-model:max="amountMax"
        @apply="applyAmountFilter"
      />
      <InvoiceStateSelect
        :states="invoiceStates"
        v-model="selectedInvoiceStates"
        @apply="applyInvoiceStateFilter"
      />
    </div>
        ...
        >
          ...
          <td>
            <select
              ...
              class="rounded px-0 py-1 text-sm font-medium text-gray-100 bg-lightBlack"
          ...
          <td class="space-x-3 text-lg">
            ...
            <button
              class="text-white hover:text-red-800"
